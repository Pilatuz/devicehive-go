...
func checkJson(t *testing.T, info interface{}, expectedJson string) {
...
func TestJsonServiceInfo(t *testing.T) {
	...
	checkJson(t, info, `{"apiVersion":"1.2.3","serverTimestamp":"2015-10-22T14:15:16.999","webSocketServerUrl":"ws://devicehive.com"}`)

	info.WebsocketUrl = ""
	info.RestUrl = "https://devicehive.com"
	checkJson(t, info, `{"apiVersion":"1.2.3","serverTimestamp":"2015-10-22T14:15:16.999","restServerUrl":"https://devicehive.com"}`)
}

// Test Equipment JSON marshaling
func TestJsonEquipment(t *testing.T) {
	equipment := core.NewEquipment("eqp-name", "eqp-code", "eqp-type")
	checkJson(t, equipment, `{"name":"eqp-name","code":"eqp-code","type":"eqp-type"}`)

	equipment.Data = "custom data"
	equipment.Id = 100
	checkJson(t, equipment, `{"id":100,"name":"eqp-name","code":"eqp-code","type":"eqp-type","data":"custom data"}`)
}
...
func TestJsonDeviceClass(t *testing.T) {
	...
	checkJson(t, deviceClass, `{"name":"class-name","version":"1.2.3","offlineTimeout":60}`)

	deviceClass.Data = "custom data"
	deviceClass.Id = 100
	checkJson(t, deviceClass, `{"id":100,"name":"class-name","version":"1.2.3","offlineTimeout":60,"data":"custom data"}`)
}
...
func TestJsonNetwork(t *testing.T) {
	...
	checkJson(t, network, `{"name":"net-name","key":"net-key","description":"custom description"}`)

	network.Description = ""
	network.Id = 100
	checkJson(t, network, `{"id":100,"name":"net-name","key":"net-key"}`)
}
...
func TestJsonDevice(t *testing.T) {
	...
	checkJson(t, device, `{"id":"dev-id","name":"dev-name","key":"dev-key","status":"Online"}`)

	device.Data = "custom data"
	checkJson(t, device, `{"id":"dev-id","name":"dev-name","key":"dev-key","status":"Online","data":"custom data"}`)

	device.Network = core.NewNetwork("net-name", "net-key")
	checkJson(t, device, `{"id":"dev-id","name":"dev-name","key":"dev-key","status":"Online","data":"custom data","network":{"name":"net-name","key":"net-key"}}`)

	device.DeviceClass = core.NewDeviceClass("class-name", "3.4.5")
	checkJson(t, device, `{"id":"dev-id","name":"dev-name","key":"dev-key","status":"Online","data":"custom data","network":{"name":"net-name","key":"net-key"},"deviceClass":{"name":"class-name","version":"3.4.5"}}`)
}
...
func TestJsonCommand(t *testing.T) {
	...
	checkJson(t, command, `{"timestamp":"2005-10-22","command":"cmd-name","parameters":"hello"}`)

	command.Id = 100
	command.Result = "custom data"
	command.Status = "done"
	checkJson(t, command, `{"id":100,"timestamp":"2005-10-22","command":"cmd-name","parameters":"hello","status":"done","result":"custom data"}`)
}

// Test Notification JSON marshaling
func TestJsonNotification(t *testing.T) {
	notification := core.NewNotification("ntf-name", "hello")
	checkJson(t, notification, `{"notification":"ntf-name","parameters":"hello"}`)

	notification.Id = 100
	checkJson(t, notification, `{"id":100,"notification":"ntf-name","parameters":"hello"}`)
}
